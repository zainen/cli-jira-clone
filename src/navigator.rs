...
impl Navigator {
    pub fn new(db: Rc<JiraDatabase>) -> Self {
      let pages: Vec<Box<dyn Page>> = vec![Box::new(HomePage { db: Rc::clone(&db) })];
        
        Self { pages, prompts: Prompts::new(), db }
    }
    ...
    pub fn handle_action(&mut self, action: Action) -> Result<()> {
        match action {
            ...
            Action::UpdateEpicStatus { epic_id } => {
                 ...
                 let new_status = (self.prompts.update_status)().ok_or_else(|| anyhow!("unknown status"))?;
